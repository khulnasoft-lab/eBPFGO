...
func (ts *xdpTestSuite) TestElfLoad() {
	...
	eb := ebpfgo.NewDefaultEbpfSystem()
	...
	txcnt := maps["txcnt"].(*ebpfgo.EbpfMap)
	ts.NotEqual(0, txcnt.GetFd())
	ts.Equal(ebpfgo.MapTypePerCPUArray, txcnt.Type)
	...
	rxcnt := maps["rxcnt"].(*ebpfgo.EbpfMap)
	ts.NotEqual(0, rxcnt.GetFd())
	ts.Equal(ebpfgo.MapTypeHash, rxcnt.Type)
	ts.Equal(8, rxcnt.KeySize)
	ts.Equal(4, rxcnt.ValueSize)
	ts.Equal(50, rxcnt.MaxEntries)

	moftxcnt := maps["match_maps_tx"].(*ebpfgo.EbpfMap)
	ts.NotEqual(0, moftxcnt.GetFd())
	ts.Equal(ebpfgo.MapTypeArrayOfMaps, moftxcnt.Type)
	ts.Equal(4, moftxcnt.KeySize)
	ts.Equal(4, moftxcnt.ValueSize)
	ts.Equal(10, moftxcnt.MaxEntries)

	mofrxcnt := maps["match_maps_rx"].(*ebpfgo.EbpfMap)
	ts.NotEqual(0, mofrxcnt.GetFd())
	ts.Equal(ebpfgo.MapTypeHashOfMaps, mofrxcnt.Type)
	...
	progmap := eb.GetMapByName("programs").(*ebpfgo.EbpfMap)
	ts.Require().NotNil(progmap)
	ts.NotEqual(0, progmap.GetFd())
	ts.Equal(ebpfgo.MapTypeProgArray, progmap.Type)
	...
	for name, program := range eb.GetPrograms() {
		// Check params
		ts.Equal(ebpfgo.ProgramTypeXdp, program.GetType())
	...
	err = xdp0.Attach(&ebpfgo.XdpAttachParams{
		Interface: "lo",
		Mode:      ebpfgo.XdpAttachModeSkb,
	})
	ts.Require().NoError(err)
	err = xdp0.Detach()
	ts.NoError(err)
...
func (ts *xdpTestSuite) TestProgramInfo() {
	...
	info, err := ebpfgo.GetProgramInfoByFd(prog.GetFd())
	ts.NoError(err)

	// Check base info
	ts.Equal(prog.GetName(), info.Name)
	ts.Equal(prog.GetFd(), info.Fd)
	ts.Equal(ebpfgo.ProgramTypeXdp, info.Type)
	...
	origMap := eb.GetMapByName("array_map").(*ebpfgo.EbpfMap)
	infoMap := info.Maps["array_map"].(*ebpfgo.EbpfMap)
	err = infoMap.Create()
	ts.NoError(err)
