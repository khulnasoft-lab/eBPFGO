...
func (ts *tcTestSuite) TestElfLoad() {
	...
	ts.FileExists(path)
	os.Remove(path)
	ts.Equal(ebpfgo.ProgramTypeSchedCls, tc1.GetType())
	...
	ts.FileExists(path)
	os.Remove(path)
	ts.Equal(ebpfgo.ProgramTypeSchedAct, tc2.GetType())
	...
	ts.FileExists(path)
	os.Remove(path)
	ts.Equal(ebpfgo.ProgramTypeSchedAct, tc3.GetType())
...
func (ts *tcTestSuite) TestProgramInfo() {
	...
	ts.Equal(ebpfgo.ProgramTypeSchedCls, info.Type)
	// Check loaded time
	now := time.Now()
