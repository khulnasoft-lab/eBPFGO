...
func (ts *mapTestSuite) TestMapLPMTrieIPv4() {
	...
	// Insert few subnets
	err = m.Insert(ebpfgo.CreateLPMtrieKey("192.168.0.0/16"), "value16")
	ts.NoError(err)
	err = m.Insert(ebpfgo.CreateLPMtrieKey("192.168.0.0/24"), "value24")
	ts.NoError(err)

	// Perform lookup (it is usually done from XDP program - here is only for integration tests)
	val1, err := m.LookupString(ebpfgo.CreateLPMtrieKey("192.168.0.10"))
	ts.NoError(err)
	ts.Equal("value24", val1)

	// Negative lookup: IP doesn't belong to any range
	_, err = m.LookupString(ebpfgo.CreateLPMtrieKey("10.10.10.10"))
	ts.Error(err)

	// "Default route" :)
	err = m.Insert(ebpfgo.CreateLPMtrieKey("0.0.0.0/0"), "value0")
	ts.NoError(err)

	// Lookup any IP
	val2, err := m.LookupString(ebpfgo.CreateLPMtrieKey("222.222.222.222"))
	ts.NoError(err)
...
func (ts *mapTestSuite) TestMapLPMTrieIPv6() {
	...
	// Insert few subnets
	err = m.Insert(ebpfgo.CreateLPMtrieKey("fafa::fbf8/120"), "value120")
	ts.NoError(err)
	err = m.Insert(ebpfgo.CreateLPMtrieKey("fafa::fbf8/125"), "val125")
	ts.NoError(err)

	// Perform lookup (it is usually done from XDP program - here is only for integration tests)
	val1, err := m.LookupString(ebpfgo.CreateLPMtrieKey("fafa::fbfa"))
	ts.NoError(err)
	ts.Equal("val125", val1)

	// Negative lookup: IP doesn't belong to any range
	_, err = m.LookupString(ebpfgo.CreateLPMtrieKey("1afa::fbfa"))
	ts.Error(err)

	// "Default route" :)
	err = m.Insert(ebpfgo.CreateLPMtrieKey("::/0"), "value0")
	ts.NoError(err)

	// Lookup any IP
	val2, err := m.LookupString(ebpfgo.CreateLPMtrieKey("1111::1111"))
	ts.NoError(err)
	ts.Equal("value0", val2)
}
...
func (ts *mapTestSuite) TestArrayOfMaps() {
	...
	mm := &ebpfgo.EbpfMap{
		Type:       ebpfgo.MapTypeArrayOfMaps,
		MaxEntries: 10,
		InnerMapFd: m.GetFd(),
	}
	err = mm.Create()
...
func (ts *mapTestSuite) TestMapFromExistingByFd() {
	...
	m2, err := ebpfgo.NewMapFromExistingMapByFd(m1.GetFd())
	ts.NoError(err)
...
func (ts *mapTestSuite) TestMapFromExistingByPath() {
	...
	m2, err := ebpfgo.NewMapFromExistingMapByPath(m1.PersistentPath)
	ts.NoError(err)
...
func (ts *mapTestSuite) TestMapPerCpuValues() {
	...
	numCpus, err := ebpfgo.GetNumOfPossibleCpus()
	ts.NoError(err)
