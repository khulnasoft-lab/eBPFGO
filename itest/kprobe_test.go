...
func (ts *kprobeTestSuite) TestElfLoad() {
	...
	perfMap := eb.GetMapByName("perf_map").(*ebpfgo.EbpfMap)
	ts.Require().NotNil(perfMap)
	ts.NotEqual(0, perfMap.GetFd())
	ts.Equal(ebpfgo.MapTypePerfEventArray, perfMap.Type)
	...
	for name, program := range eb.GetPrograms() {
		// Check params
		ts.Equal(ebpfgo.ProgramTypeKprobe, program.GetType())
...
func (ts *kprobeTestSuite) TestKprobeEvents() {
	...
	perfEvents, err := ebpfgo.NewPerfEvents(perfMap)
	ts.Require().NoError(err)
