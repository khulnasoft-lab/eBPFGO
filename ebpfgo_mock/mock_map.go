...
func init() {
	...
	for res := C.bpf_map_get_head(); res != nil; res = C.bpf_map_get_next(res) {
		...
		m := &MockMap{
			...
			Name:       C.GoString(res.name),
			Type:       ebpfgo.MapType(res.map_def.map_type),
...
func (m *MockMap) CloneTemplate() ebpfgo.Map {
	res := *m
	res.fd = nil
...
func (m *MockMap) Lookup(ikey interface{}) ([]byte, error) {
	...
	key, err := ebpfgo.KeyValueToBytes(ikey, m.KeySize)
	if err != nil {
		return nil, err
	}
...
func (m *MockMap) LookupString(ikey interface{}) (string, error) {
	...
	return ebpfgo.NullTerminatedStringToString(val), nil
}
...
func (m *MockMap) LookupUint64(ikey interface{}) (uint64, error) {
	...
	return ebpfgo.ParseFlexibleIntegerLittleEndian(val), nil
}
...
func (m *MockMap) Insert(ikey interface{}, ivalue interface{}) error {
	...
	val, err := ebpfgo.KeyValueToBytes(ivalue, m.ValueSize)
	if err != nil {
		return err
	}
...
func (m *MockMap) GetNextKeyString(ikey interface{}) (string, error) {
	...
	return ebpfgo.NullTerminatedStringToString(nextKey), nil
}
...
func (m *MockMap) GetNextKeyUint64(ikey interface{}) (uint64, error) {
	...
	return ebpfgo.ParseFlexibleIntegerLittleEndian(nextKey), nil
}
...
func (m *MockMap) GetType() ebpfgo.MapType {
	return m.Type
}
