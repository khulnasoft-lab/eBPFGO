...
func main() {
	...
	// cleanup old probes
	if err := ebpfgo.CleanupProbes(); err != nil {
		log.Println(err)
	}
	...
	go func(events <-chan []byte) {
		...
		for {
			...
			if b, ok := <-events; ok {
				...
				ts := ebpfgo.KtimeToTime(ev.KtimeNs)
				fmt.Printf("%s  %-16s  %-6d %-6d %-6d %s\n",
					ts.Format("15:04:05.000"),
					ebpfgo.NullTerminatedStringToString(ev.Comm[:]),
...
func (p *Program) AttachProbes() error {
	...
	// create perf events
	var err error
	p.pe, err = ebpfgo.NewPerfEvents(m)
	if err != nil {
		return err
	}
